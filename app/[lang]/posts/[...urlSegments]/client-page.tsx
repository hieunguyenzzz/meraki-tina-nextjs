...
// TinaCMS imports temporarily removed for build
...
// Temporary helper functions until TinaCMS is fully integrated
const tinaField = (obj: any, field: string) => `${field}`;

export default function PostClientPage(props: ClientPostProps) {
  const { theme } = useLayout();
  // const { data } = useTina({ ...props }); // Temporarily disabled
  const { data } = props;
    ...
    <ErrorBoundary>
      <Section>
        ...
        <div data-tina-field={tinaField(post, '_body')} className='prose dark:prose-dark w-full max-w-none'>
          {/* TinaMarkdown temporarily replaced with simple content rendering */}
          <div>
            {post._body ? "Post content will be rendered here" : "No content available"}
          </div>
        </div>
      </Section>
    </ErrorBoundary>
